...
def parse_data(data):                                              # получение данных в виде строки и преобразование ее в список  
    math_equation = []                  
    math_elem = []
    for i in data:
        if i.isdigit():
...
def calculate(part_list):                                           # расчет
    result = 0
    if len(part_list) == 1:
        return part_list[0]
    for s in part_list:
...
def data_calculation(lst):                                            # расчет данных, в случае наличия скобок - сначала раскрываем скобки
    flag = 1
    while flag == 1:
        if ')' in lst:
